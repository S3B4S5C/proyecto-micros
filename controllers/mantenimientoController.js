...
export const nuevoMantenimiento = async (req, res) => {
  const { descripcion, interno, token, tipo_mantenimiento } = req.body;
  let { fecha } = req.body;
  ...
  try {
    ...
    res.status(201).json({
      ...
      mantenimiento: mantenimientoRegistrado,
    });
  } catch (error) {
    res
      .status(500)
...
export const getMantenimientosPorLinea = async (req, res) => {
  ...
  try {
    ...
    if (!mantenimientos.length) {
      return res
        .status(404)
        .json({ message: "No se encontraron mantenimientos para esta linea" });
    }
    res.status(200).json(mantenimientos);
  } catch (error) {
    res.status(500).json({
      message: "Error al obtener los mantenimientos por linea",
      error: error.message,
    });
  }
};
...
export const getMantenimientosPorMicros = async (req, res) => {
  ...
  try {
    ...
    if (!micro) {
      return res
        .status(404)
        .json({ message: "Micro no encontrado para esta línea" });
    }
    const mantenimientos = await model.mantenimiento.findAll({
      include: [
        {
    ...
    if (!mantenimientos.length) {
      return res
        .status(404)
        .json({ message: "No se encontraron mantenimientos para este micro" });
    }
    res.status(200).json(mantenimientos);
  } catch (error) {
    res.status(500).json({
      message: "Error al obtener los mantenimientos por micro",
      error: error.message,
    });
  }
};
