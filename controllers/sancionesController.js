...
export const registrarFichaSancion = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({
      message: "Ficha de Sanción registrada con éxito",
...
export const getFichaSancionPorLinea = async (req, res) => {
  ...
  try {
    ...
    if (!fichas.length) {
      return res
        .status(404)
        .json({ message: "No se encontraron fichas de sancion " });
    }
    res.status(200).json(fichas);
  } catch (error) {
    res.status(500).json({
      message: "Error al obtener las fichas de sancion de los micros",
      error: error.message,
    });
  }
};
...
export const getFichaSancionChofer = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      message: "Error al obtener las fichas de sancion del chofer",
      error: error.message,
    });
  }
};
